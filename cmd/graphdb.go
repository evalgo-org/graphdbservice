	...
	case "repo-migration":
		...
		for _, bind := range srcGraphDB.Results.Bindings {
			if bind.Id["value"] == task.Src.Repo {
				foundRepo = true
				fmt.Println(db.GraphDBRepositoryConf(task.Src.URL, task.Src.Username, task.Src.Password, bind.Id["value"]))
				fmt.Println(db.GraphDBRepositoryBrf(task.Src.URL, task.Src.Username, task.Src.Password, bind.Id["value"]))
			}
		}
		if !foundRepo {
			result["message"] = "could not find required src repository " + task.Src.Repo
			result["src_repo"] = task.Src.Repo
			result["tgt_repo"] = task.Tgt.Repo
			return result, errors.New("could not find required src repository " + task.Src.Repo)
		}
		...
		for _, bind := range tgtGraphDB.Results.Bindings {
			if bind.Id["value"] == task.Src.Repo {
				db.GraphDBDeleteRepository(task.Tgt.URL, task.Tgt.Username, task.Tgt.Password, task.Src.Repo)
			}
		}
		confFile := task.Src.Repo+".ttl"
		dataFile := task.Src.Repo+".brf"
		db.GraphDBRestoreConf(task.Tgt.URL, task.Tgt.Username, task.Tgt.Password, confFile)
		db.GraphDBRestoreBrf(task.Tgt.URL, task.Tgt.Username, task.Tgt.Password, dataFile)
		os.Remove(confFile)
		os.Remove(dataFile)
